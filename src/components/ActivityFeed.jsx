...
function MergedGroup({ group, onDelete, onEntryUpdate, userId }) {
  ...
  const handleEntryUpdate = (id, patch) => {
    ...
    setLocalTotals((prev) => {
      ...
      return {
        emitted: parseFloat(
          Math.max(0, prev.emitted - oldEmitted + patch.co2_score_kg).toFixed(
            2,
          ),
        ),
        prevented: parseFloat(
          Math.max(
            0,
            prev.prevented - oldPrevented + patch.co2_prevented_kg,
          ).toFixed(2),
        ),
      };
    });
  ...
  return (
    <details className="mb-2 border border-gray-200 rounded-lg bg-white shadow-sm open:shadow-md transition-shadow">
      <summary
        className="p-3 font-semibold flex justify-between items-center cursor-pointer list-none select-none
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`tabular-nums text-xs font-semibold ${IMPACT(co2Val)}`}
                    >
                      +{co2Val.toFixed(2)} kg
                    </span>
                  </div>
